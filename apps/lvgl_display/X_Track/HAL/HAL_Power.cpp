...
#if DT_NODE_EXISTS(DT_ALIAS(poweron)) && \
    DT_NODE_EXISTS(DT_ALIAS(bat_ch)) && \
    DT_NODE_EXISTS(DT_PATH(zephyr_user)) && \
    DT_NODE_HAS_PROP(DT_PATH(zephyr_user), io_channels)
    
    static const struct gpio_dt_spec PowerOn_DT = GPIO_DT_SPEC_GET(DT_ALIAS(poweron), gpios);
    static const struct gpio_dt_spec BatCh_DT = GPIO_DT_SPEC_GET(DT_ALIAS(bat_ch), gpios); 
...
void HAL::Power_Init(void)
{
    LOG_ERR("Power init fail, Device Tree is Err");
}
