...
void LiveMapView::Map_Create(lv_obj_t* par, uint32_t tileNum)
{
    ...
    ui.map.imgTiles = (lv_obj_t**)lv_zalloc (tileNum * sizeof(lv_obj_t*));
    ...
    if (img_dsc) {
        lv_obj_set_pos(img, -img_dsc->header.w, -img_dsc->header.h);
    }
    ui.map.imgArrow = img;
}
...
{
    lv_point_precise_t end_point;
    if (!ui.track.lineTrack->get_end_point(&end_point))
    {
        return;
    }

    lv_point_precise_t precise_points[2];
    precise_points[0].x = ui.track.pointActive[0].x;
    precise_points[0].y = ui.track.pointActive[0].y;
    precise_points[1].x = ui.track.pointActive[1].x;
    precise_points[1].y = ui.track.pointActive[1].y;

    lv_line_set_points(ui.track.lineActive, precise_points, 2);
}
