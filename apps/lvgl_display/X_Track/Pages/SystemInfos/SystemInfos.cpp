...
LOG_MODULE_REGISTER(systeminfo, LOG_LEVEL_DBG);
...
void SystemInfos::Update()
{
    ...
    // LOG_INF("lat:%f, lng:%f, alt:%f, course:%f, speed:%f",
    //         lat,lng,alt,course,speed);
...
{
    SystemInfos* instance = (SystemInfos*)lv_timer_get_user_data(timer);
...
void SystemInfos::onEvent(lv_event_t* event)
{
    ...
    if (code == LV_EVENT_PRESSED)
    {
        if (lv_obj_has_state(obj, LV_STATE_FOCUSED))
        {
            ...
            LOG_INF("SystemInfos pop succeeded");
        }
    }
